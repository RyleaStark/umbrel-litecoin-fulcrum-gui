...
describe("createConnectionDetails", () => {
  it("creates distinct Litecoin LAN and Tor connection records", () => {
    expect(createConnectionDetails({ localHost: "umbrel.local", torHost: "fulcrum.example.onion", port: "51002" })).toEqual({
      ...
      tor: { address: "fulcrum.example.onion", port: 51002, connectionString: "fulcrum.example.onion:51002", transport: "tcp" }
    });
  });

  it("never adds Electrum transport suffixes to user-facing connection strings", () => {
    const details = createConnectionDetails({ localHost: "umbrel.local", torHost: "fulcrum.example.onion", port: 51002 });
